...
def create_video(model_name, ts):
    img_list = []
    base_path = os.getcwd()
    temp = glob.glob(base_path+"/_sol/%s/bp/*.png" % (ts))
    ...
    out = cv2.VideoWriter(base_path+"/_sol/%s/video.avi" % (ts), fourcc, 20, size)
    for i in range(len(img_list)):
        out.write(img_list[i])
    out.release()

def plot_steering_cmd(steering_list, model_name, ts):
    frame_list = list(range(len(steering_list)))
    plt.plot(steering_list, '-bx')
    plt.xlabel('frame')
    plt.ylabel('steering cmd')
    # ax = plt.gca()
    # ax.set_ylim([0, 0.1])
    plt.title('Steering Command vs. Frame');
    base_path = os.getcwd()
    
    plt.savefig(base_path+"/_sol/%s/steering.png" % (ts))

def main():
    ts = str(time.time())
    steering_cmds = pd.DataFrame()
    ...
    try:
        ...
        # Load nn-model
        model = ResNet.load_from_checkpoint('checkpoints/2022_10_09_215701_DS1_ResNet/ResNet-epoch=0057-MAE_Val=0.03347608.ckpt')
        ...
        if not os.path.isdir("_sol/%s" % (ts)):
            os.makedirs("_sol/%s" % (ts))
        ...
        rgb_bp_image_queue = queue.Queue()
        camera_rgb.listen(rgb_bp_image_queue.put)
        ...
        for frame in range(300):
            ...
            if frame  > 10:
                ...
                rgb_bp_image = rgb_bp_image_queue.get()
                
                rgb_image_mid.save_to_disk("_sol/%s/rgb_mid-%s.png" % (ts, frame))
                rgb_image_left.save_to_disk("_sol/%s/rgb_left-%s.png" % (ts, frame))
                rgb_image_right.save_to_disk("_sol/%s/rgb_right-%s.png" % (ts, frame))
                depth_image.save_to_disk("_sol/%s/depth-%s.png" % (ts, frame), cc)
                rgb_bp_image.save_to_disk("_sol/%s/bp/%s.png" % (ts, frame))
                ...
                gray_mid = Image.open("_sol/%s/rgb_mid-%s.png" % (ts, frame)).convert("L")
                gray_left = Image.open("_sol/%s/rgb_left-%s.png" % (ts, frame)).convert("L")
                gray_right = Image.open("_sol/%s/rgb_right-%s.png" % (ts, frame)).convert("L")
                depth = Image.open("_sol/%s/depth-%s.png" % (ts, frame))


                tensor_gray_mid = torchvision.transforms.functional.to_tensor(gray_mid)
                tensor_gray_left = torchvision.transforms.functional.to_tensor(gray_left)
                tensor_gray_right = torchvision.transforms.functional.to_tensor(gray_right)
                

                depth_image = torchvision.transforms.functional.to_tensor(depth)
                image = torch.cat((tensor_gray_mid, tensor_gray_left, tensor_gray_right, depth_image), 0).unsqueeze(0)
                output = model.predict(image)[0].cpu().detach().numpy()
                steering_list.append(float(output[0]))
                vehicle.apply_control(carla.VehicleControl(throttle=float(output[1]), steer=float(output[0])))
                ...
                new_row = pd.DataFrame(
                    [[frame, output]], columns=["frame", "steercmd"])
                steering_cmds = steering_cmds.append(
                    new_row, ignore_index=True)
                    # pdb.set_trace()

        # time.sleep(5)

        steering_cmds.to_csv("_sol/%s/steering_cmd.csv" % (ts ))
        # create_video(model_name, ts)
        # plot_steering_cmd(steering_list, model_name, ts)
